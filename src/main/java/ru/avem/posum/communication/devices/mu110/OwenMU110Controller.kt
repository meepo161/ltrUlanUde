...
class OwenMU110Controller(private val unitID: UnitID, observer: Observer) : Observable(), Device {
    companion object {
        ...
        var isRestartNeed = false
        var isStartNeed = false
    }
    ...
    fun on2KM() {
        try {
            ...
            thread {
                while (isRestartNeed) {
                    ModbusConnection.master!!.writeMultipleRegisters(unitID.id, 50, arrayOf(SimpleRegister(2.toShort())))
                }
            }
        } catch (e: Exception) {
            isResponding = false
    ...
    fun on3KM() {
        try {
            ...
            thread {
                while (isStartNeed) {
                    ModbusConnection.master!!.writeMultipleRegisters(unitID.id, 50, arrayOf(SimpleRegister(4.toShort())))
                }
            }
        } catch (e: Exception) {
            isResponding = false

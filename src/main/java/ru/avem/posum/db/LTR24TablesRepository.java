...
public class LTR24TablesRepository extends DataBaseRepository {
    ...
    public static void insertLTR24Table(LTR24Table ltr24Table) {
        sendAction((ltr24ModuleDao) -> ltr24ModuleDao.create(ltr24Table));
    }

    public static void updateLTR24Module(LTR24Table ltr24Table) {
        sendAction((ltr24ModuleDao) -> ltr24ModuleDao.update(ltr24Table));
    }

    public static void deleteLTR24Module(LTR24Table ltr24Table) {
        sendAction((ltr24ModuleDao) -> ltr24ModuleDao.delete(ltr24Table));
    }
    ...
    public static List<LTR24Table> getAllLTR24Tables(){
        ...
        return (List<LTR24Table>) ltr24Modules[0];
    }

    public static LTR24Table getLTR24Module(long id) {
        final LTR24Table[] ltr24Table = {null};
        sendAction((ltr24ModulesDao -> ltr24Table[0] = ltr24ModulesDao.queryForId(id)));
        return ltr24Table[0];
    }

    @FunctionalInterface
    private interface Actionable {
        void onAction(Dao<LTR24Table, Long> ltr24ModulesDao) throws SQLException;
    }

    private static void sendAction(LTR24TablesRepository.Actionable actionable) {
        try (ConnectionSource connectionSource = new JdbcConnectionSource(DATABASE_URL)) {
            Dao<LTR24Table, Long> ltr24ModuleDao = DaoManager.createDao(connectionSource, LTR24Table.class);
